...
class CreateCurrencyExchangeRatesJob < ApplicationJob
  ...
  def perform(day)
    ...
    CurrencyExchangeRate.create_or_update(historical_data(date))
  ...
  def fetch_and_save_historical_data(date)
    ...
    client.historical(date.strftime('%Y-%m-%d'))
    date = data['date'].to_date
    currency_exchange_rate_records = []
    data['rates'].each do |target_currency, rate|
      record = CurrencyExchangeRate.find_or_initialize_by(date: date, base_currency: base_currency, target_currency: target_currency)
      record.rate = rate
